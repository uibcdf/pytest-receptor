...
| PR-PILOT-016 | Low | `uibcdf/pytest-receptor#4`: the late-terminal `/dev/null` writer remained open, leaking `ResourceWarning` after successful runs | Close it after all `pytest_unconfigure` implementations using a first-entered hook wrapper, preserving PR-PILOT-011's late-write safety | post | 1 | **done 2026-09-26** |
...
| PR-REL-008 | Medium | Package and CI metadata stop at Python 3.13 | Demonstrate Python 3.14 across the pytest 8/9 matrix, then align metadata, packaging, documentation, public release, and clean-install evidence under `uibcdf/molsyssuite#29` | post | 4 | **done 2026-09-21** |
...
Filed [pytest#14724](https://github.com/pytest-dev/pytest/issues/14724): the
`--no-summary` finding had lived only inside a comment on our own proposal,
where nobody looking for it would find it. Verified against 8.4.2 and 9.1.1 and
reproduced without our plugin present, so the report stands on pytest and
pytest-cov alone.

Closed pytest#14710 ourselves. We had written "I am happy for this issue to be
closed" and then left it open, which asks a volunteer maintainer to read a long
thread to reach a conclusion we had already reached. Our own retraction is our
own to clean up.

Held back from opening anything further upstream: #14720 has had no response in
four days, and three reports in one week from a newcomer fills a volunteer queue
faster than anyone can drain it. We do not yet know whether our reporting format
works for them.

Added trove classifiers and an explicit licence. pytest's plugin list is
generated by scanning the PyPI simple index for `pytest-` and `pytest_` names,
so listing needs no classifier, but the status column reads `Development
Status`, and we shipped none. Declared Beta: 0.6 is pre-1.0 and the format may
still change. Publication to PyPI is deferred -- we are not ready.
...
*same* warning forty times and therefore could not detect that only three of
sixty groups were being reported on a real suite. The replacement varied its
warnings by number -- and the numeric normalization added later for warning
grouping collapsed all forty back into one group. The scenario written to
replace the one-group scenario silently became the one-group scenario.

Nothing failed. The row simply improved, from -63.8% to -97.4%, and improvement
is not something a test suite flags. It was caught only because the figure was
being re-measured before publication and the change was too large to be real.

Fixed by varying the warnings by a non-numeric token. The row returns to -64.6%.
Every published figure -- README, `docs/benchmarks.md` -- was regenerated from
the corrected run rather than patched in the one row that moved.

Third occurrence of the same class of defect, after the warning scenario itself
and the cascade token budget: **a test can pass while measuring the wrong axis,
and normalization is a silent way to change which axis a fixture varies along**.
